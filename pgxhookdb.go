...
// BufSize Set pglogrus.BufSize = <value> _before_ calling NewHook
// Once the buffer is full, logging will start blocking, waiting for slots to
// be available in the queue new_doc
...
type Hook struct {
	...
	db         *pgxpool.Pool
	mu         sync.RWMutex
	InsertFunc func(*pgxpool.Pool, *logrus.Entry) error
	filters    []filter
}

// AsyncHook to sent logs to PostgreSQL in transaction
type AsyncHook struct {
	*Hook
	...
	InsertFunc func(pgx.Tx, *logrus.Entry) error
}

var insertFunc = func(db *pgxpool.Pool, entry *logrus.Entry) error {
	jsonData, err := json.Marshal(entry.Data)
	if err != nil {
		return err
	}

	_, err = db.Exec(context.Background(), "INSERT INTO adm.logs(level_id, message, message_data) VALUES ($1,$2,$3)", entry.Level, entry.Message, jsonData)
	return err
}

var asyncInsertFunc = func(txn pgx.Tx, entry *logrus.Entry) error {
	jsonData, err := json.Marshal(entry.Data)
	if err != nil {
		return err
	}

	_, err = txn.Exec(context.Background(), "INSERT INTO adm.logs(level_id, message, message_data) VALUES ($1,$2,$3)", entry.Level, entry.Message, jsonData)
	return err
}

type filter func(*logrus.Entry) *logrus.Entry

// NewHook creates a PGXHook to be added to an instance of logger.
func NewHook(db *pgxpool.Pool, extra map[string]interface{}) *Hook {
	return &Hook{
		Extra:      extra,
		db:         db,
...
func NewAsyncHook(db *pgxpool.Pool, extra map[string]interface{}) *AsyncHook {
	hook := &AsyncHook{
...
// but don't want pgx to store them.
...
// FlushEvery sets the internal hook ticker.
...
func (hook *AsyncHook) fire() {
	for {
		var err error
		txn, err := hook.db.Begin(context.Background())
		if err != nil {
